...
# Modified dataset utilities to support both PointNetVLAD and Chilean datasets
...
def make_datasets(params: TrainingParams, validation: bool = True):
    """创建训练和验证数据集，支持多种数据集类型"""
    datasets = {}
    train_set_transform = TrainSetTransform(params.set_aug_mode)

    # 检测数据集类型
    dataset_type = detect_dataset_type(params)

    if dataset_type == 'chilean':
        # Chilean数据集
        train_transform = ChileanTrainTransform(params.aug_mode)
        datasets['train'] = ChileanTrainingDataset(params.dataset_folder, params.train_file,
                                                   transform=train_transform, set_transform=train_set_transform)
        if validation and params.val_file is not None:
            datasets['val'] = ChileanTrainingDataset(params.dataset_folder, params.val_file)
    else:
        # PointNetVLAD数据集（Oxford RobotCar和Inhouse）
...
def detect_dataset_type(params: TrainingParams) -> str:
    """根据配置参数检测数据集类型"""
    # 检查数据集文件夹路径
    if 'chilean' in params.dataset_folder.lower():
        return 'chilean'

    # 检查训练文件名
    if 'chilean' in params.train_file.lower():
        return 'chilean'

    # 检查配置文件路径
    if hasattr(params, 'params_path') and 'chilean' in params.params_path.lower():
        return 'chilean'
...
def make_dataloaders(params: TrainingParams, validation=True):
    ...
    dataloaders['train'] = DataLoader(datasets['train'], batch_sampler=train_sampler,
...
def filter_query_elements(query_set: List[EvaluationTuple], map_set: List[EvaluationTuple],
    ...
    """函数用于评估数据集生成，过滤掉在dist_threshold阈值内没有对应地图元素的查询元素"""
    # 检测数据维度
    if len(map_set) > 0:
        pos_dim = len(map_set[0].position)
        if pos_dim == 3:
            # 3D数据集（如Chilean）
